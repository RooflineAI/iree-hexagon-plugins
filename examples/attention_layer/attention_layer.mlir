...
module @module {
  util.func public @main$async(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view, %arg2: !hal.buffer_view, %arg3: !hal.fence, %arg4: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    %cst = arith.constant dense<0.000000e+00> : tensor<4x1024x1024xf32>
    ...
    ^bb0(%in: f32, %out: i1):
      %36 = arith.extf %in : f32 to f64
      %37 = arith.cmpf oeq, %36, %cst_2 : f64
      linalg.yield %37 : i1
    ...
    ^bb0(%in: i1, %out: i1):
      %36 = arith.andi %in, %out : i1
      linalg.yield %36 : i1
    } -> tensor<4x1024x1xi1>
    %29 = linalg.generic {indexing_maps = [#map2, #map, #map, #map], iterator_types = ["parallel", "parallel", "parallel"]} ins(%28, %cst, %23 : tensor<4x1024x1xi1>, tensor<4x1024x1024xf32>, tensor<4x1024x1024xf32>) outs(%10 : tensor<4x1024x1024xf32>) {
    ^bb0(%in: i1, %in_4: f32, %in_5: f32, %out: f32):
      %36 = arith.select %in, %in_4, %in_5 : f32
      linalg.yield %36 : f32
    } -> tensor<4x1024x1024xf32>
    %30 = linalg.fill ins(%cst_0 : f32) outs(%3 : tensor<4x1024x128xf32>) -> tensor<4x1024x128xf32>
    %31 = linalg.batch_matmul ins(%29, %6 : tensor<4x1024x1024xf32>, tensor<4x1024x128xf32>) outs(%30 : tensor<4x1024x128xf32>) -> tensor<4x1024x128xf32>
    %32 = tensor.empty() : tensor<4x1024x128xf16>
    %33 = linalg.generic {indexing_maps = [#map, #map], iterator_types = ["parallel", "parallel", "parallel"]} ins(%31 : tensor<4x1024x128xf32>) outs(%32 : tensor<4x1024x128xf16>) {
    ^bb0(%in: f32, %out: f16):
      %36 = arith.truncf %in : f32 to f16
      linalg.yield %36 : f16
    } -> tensor<4x1024x128xf16>
    %34 = hal.tensor.barrier join(%33 : tensor<4x1024x128xf16>) => %arg4 : !hal.fence
    %35 = hal.tensor.export %34 : tensor<4x1024x128xf16> -> !hal.buffer_view

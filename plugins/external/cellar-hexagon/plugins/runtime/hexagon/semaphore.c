...
//
// The current implementation works at the host side only. It is built on the
// centralized async semaphore infrastructure (mirroring the "local_sync"
// driver): waits use the async semaphore futex-based mechanism and signals
// dispatch timepoints directly.
//===----------------------------------------------------------------------===//
...
iree_status_t iree_hal_hexagon_semaphore_create(
  ...
  *out_semaphore = NULL;
  ...
  iree_async_semaphore_initialize(
      (const iree_async_semaphore_vtable_t *)&iree_hal_hexagon_semaphore_vtable,
...
static void
iree_hal_hexagon_semaphore_destroy(iree_async_semaphore_t *base_semaphore) {
  iree_hal_hexagon_semaphore_t *semaphore =
      iree_hal_hexagon_semaphore_cast(iree_hal_semaphore_cast(base_semaphore));
...
static uint64_t
iree_hal_hexagon_semaphore_query(iree_async_semaphore_t *base_semaphore) {
...
iree_hal_hexagon_semaphore_signal(iree_async_semaphore_t *base_semaphore,
                                  uint64_t new_value,
...
iree_hal_hexagon_semaphore_wait(iree_hal_semaphore_t *base_semaphore,
  ...
  // futex-based notification with targeted per-semaphore wakeups and proper
  // deadline handling.

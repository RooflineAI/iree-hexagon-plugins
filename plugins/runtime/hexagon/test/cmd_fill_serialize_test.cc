...
TEST(CmdFillSerializeTest, SerializeDirectBufferWithFourBytePattern) {
  ...
  iree_hal_buffer_ref_t dest = iree_hal_make_buffer_ref(
      &hexagon_test_direct_buffer1, /*offset=*/24, /*length=*/80);
  ...
  EXPECT_EQ(HEXAGON_TEST_DIRECT_BUFFER1_FD, cmd_fill->trgt.fd);
  EXPECT_EQ(dest.offset, cmd_fill->trgt.offset);
  EXPECT_EQ(dest.length, cmd_fill->trgt.length);
}

TEST(CmdFillSerializeTest, SerializeDirectSubspanBufferWithTwoBytePattern) {
  const std::array<uint8_t, 2> pattern = {0x66, 0x77};
  iree_hal_buffer_ref_t dest = iree_hal_make_buffer_ref(
      &hexagon_test_direct_subspan_buffer0, /*offset=*/24, /*length=*/80);
  ...
  EXPECT_EQ(dest.buffer_slot, cmd_fill->trgt.slot);
  EXPECT_EQ(HEXAGON_TEST_DIRECT_SUBSPAN_BUFFER0_FD, cmd_fill->trgt.fd);
  EXPECT_EQ(HEXAGON_TEST_DIRECT_SUBSPAN_BUFFER0_OFFSET + dest.offset,
            cmd_fill->trgt.offset);
  EXPECT_EQ(dest.length, cmd_fill->trgt.length);
}
